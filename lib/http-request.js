...
function createHttpRequest(deps) {
	...
	return function request(requestOptions, callback, fetchOptions) {
		...
		function dispatch(uri, redirects, previousUrl) {
			var parsed;
			try {
				parsed = new URL(uri);
			} catch (err) {
			...
			var headers = redirectHeaders(requestOptions.headers || {}, previousUrl, parsed);
			var outgoing;
			try {
...
function isPublicAddress(address) {
	...
	if (family === 6 && !ipv6GlobalUnicast.check(address, 'ipv6')) {
		return false;
	}
...
function buildIpv6GlobalUnicast() {
